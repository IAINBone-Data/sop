...
    
    // Menangani tanggal revisi (hanya tampil jika ada)
...
        if (statusLower === 'berlaku' || statusLower === 'aktif') {
...
    }
    
    // --- LOGIKA PRATINJAU & UNDUH FILE ---
    const fileUrl = item.File || '';
    let fileId = null;

    // Regex yang lebih baik untuk mengekstrak ID dari berbagai format URL Google Drive
    const driveRegex = /\/file\/d\/([a-zA-Z0-9-_]+)|uc\?id=([a-zA-Z0-9-_]+)|open\?id=([a-zA-Z0-9-_]+)/;
    const match = fileUrl.match(driveRegex);
    
    // ID bisa berada di grup tangkapan (capture group) yang berbeda, jadi kita cek semuanya
    if (match) {
        fileId = match[1] || match[2] || match[3];
    }

    // Reset tampilan preview setiap kali fungsi ini dipanggil
    DOM.tablePreviewContainer.classList.add('hidden');
    DOM.tablePreviewContent.innerHTML = '';
    DOM.detailDownloadLink.style.display = 'inline-block'; // Tampilkan tombol unduh default

    if (fileId) {
        // Atur link unduh jika ID file ditemukan
        DOM.detailDownloadLink.href = `https://drive.google.com/uc?export=download&id=${fileId}`;
        
        // Cek format file untuk menampilkan pratinjau PDF
...
        }
    } else {
        // Jika tidak ada fileId, nonaktifkan link unduh
        DOM.detailDownloadLink.href = '#';
        // Opsional: Sembunyikan tombol jika tidak ada file sama sekali
        // DOM.detailDownloadLink.style.display = 'none';
    }
